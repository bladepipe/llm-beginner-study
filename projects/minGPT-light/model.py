"""
minGPT model.py —— 中文注释版（精简教学用）
【进阶对比】这是 Bigram 的‘升级版’：不再只看前1个字符，而是看整个上文（self-attention）
"""
import torch
...
class CausalSelfAttention(nn.Module):
    """
    【核心升级】Bigram 只记住前1个字符；这里用 attention 记住全部上文
    """
...
class Block(nn.Module):
    """
    【模块化设计】每个 Block = Attention + MLP，像乐高一样堆叠
    """
    def __init__(self, config):
        super().__init__()
...
class GPT(nn.Module):
    """
    【完整模型】输入文本 → 嵌入 → 多个 Block → 输出 logits
    """
    def __init__(self, config):
        super().__init__()
